-- Rappel annuel auto-évaluation santé (axe 4) : 2 vagues (1er octobre, 1er
-- décembre) pour les users actifs sans cp_action 'auto_evaluation' sur
-- l'année en cours. Dédup via metadata (kind/year/wave) : un re-run ne crée
-- aucun doublon. Canal in-app uniquement (table notifications, type='in_app').
...
select cron.schedule('autoeval_reminder_oct', '0 8 1 10 *', $$select public.send_autoeval_reminders('oct')$$);
select cron.schedule('autoeval_reminder_dec', '0 8 1 12 *', $$select public.send_autoeval_reminders('dec')$$);
