-- Rappel Autopilot : relance mi-mois + fin de mois si le plan du mois
-- (autopilot_plan) contient des items status='todo'. Cloche in-app
-- inconditionnelle (pas de dépendance à user_notification_preferences ici —
-- ce gate n'intervient que côté Edge Function, pour le canal push).
-- Dédup via metadata (kind/month_key/wave) : un re-run ne crée aucun doublon.
-- Même pattern que send_autoeval_reminders (20260716f) : SECURITY DEFINER,
-- search_path fixé, relais push additif après l'INSERT.
  ...
  ins as (
    ...
    select
      ...
      'Il te reste ' || f.todo_count || ' action' || (case when f.todo_count > 1 then 's' else '' end)
        ...
        || ' ce mois-ci dans ton plan Sophie. Quelques minutes suffisent pour avancer.',
      'sent',
      now(),
...
select cron.schedule('autopilot_reminder_mid', '0 8 15 * *', $$select public.send_autopilot_reminders('mid')$$);
select cron.schedule('autopilot_reminder_end', '0 8 28 * *', $$select public.send_autopilot_reminders('end')$$);
