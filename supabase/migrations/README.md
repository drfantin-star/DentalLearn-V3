...
| 2026-04-28 | `20260428_news_syntheses_failed_status.sql` (+ rollback) | `claude/news-section-ticket-5-hPm1t` | Section News Phase 0bis Ticket 5 — extension `news_syntheses.status` à `failed`/`failed_permanent` (CHECK renommé `news_syntheses_status_extended_check`, anti-Lz6) + 3 colonnes traces (`validation_errors` jsonb, `validation_warnings` jsonb, `failed_attempts` int NOT NULL DEFAULT 0) + index `news_syntheses_status_idx`. Support arbitrages produit A2/A3/A4 (retry × 2 puis fail, cap retries=2 → `failed_permanent`). Addendum à v1.3 §5.2, autorisé Dr Fantin pour éviter boucle infinie sur articles cassés. |
| 2026-04-28 | `20260428_news_synthesize_articles_cron.sql` (+ rollback) | `claude/news-section-ticket-5-hPm1t` | Section News Ticket 5 — ajout du job cron `news_synthesize_articles` (lundi 20h00 UTC = 22h00 Paris été), 6h après `score_articles` pour laisser le temps à Dr Fantin d'auditer la sélection avant synthèse Sonnet. Body POST `{"limit": 8}` (cap dur code MAX_BATCH_LIMIT=15, marge IDLE_TIMEOUT 150s avec Sonnet ~3× plus lent que Haiku). 5e job hebdo de la chaîne News Phase 1. Convention UTC héritée Tickets 2-4. |
| 2026-04-29 | `20260429_news_synthesize_articles_cron_relimit.sql` (+ rollback) | `claude/news-section-ticket-5-hPm1t` | Section News Ticket 5 hot fix — recalibrage body cron `news_synthesize_articles` de `{"limit": 8}` → `{"limit": 3}` après mesure live ~30s/article (vs ~8s estimés en design) qui faisait dépasser IDLE_TIMEOUT 150s sur 5+ articles. Schedule `'0 20 * * 1'` inchangé. Limites alignées sur DEFAULT_BATCH_LIMIT=3 / MAX_BATCH_LIMIT=5 dans types.ts (mesure : 3 articles ≈ 90s, marge 60s). DOWN restaure `{"limit": 8}` si retour en arrière nécessaire. |
| 2026-04-29 | `20260429_news_synthesize_articles_cron_second.sql` (+ rollback) | `claude/news-section-ticket-5-hPm1t` | Section News Ticket 5 hot fix — ajout d'un 2e job cron `news_synthesize_articles_late` (lundi 22h00 UTC = 00h00 Paris été), 2h après le 1er. Migration ADDITIVE — ne touche pas au cron existant. Rationale : avec le recalibrage à `{"limit": 3}`, 1 cron seul absorbe 3 articles/sem alors que le rythme stationnaire mesuré est ~6,5/sem. 2 crons × 3 = 6/sem, quasi-aligné. Le LEFT JOIN news_syntheses du loadCandidates exclut automatiquement les articles déjà absorbés au 1er run. 6e job hebdo de la chaîne News Phase 1. |
| 2026-05-18 | `20260518_fix_pubmed_cpu_split_crons.sql` (+ rollback) | `claude/dentallearn-setup-FY4BM` | Fix CPU limit Edge Function `ingest_pubmed` (2000 ms saturé sur 10 sources en séquentiel, 6 sources jamais fetchées depuis 2 semaines). Drop du cron monolithique `news_ingest_pubmed`, remplacé par 10 crons individuels (`news_ingest_pubmed_<slug>`) décalés de 2 min de 04:00 à 04:18 UTC le lundi. Chaque cron POSTe `{"source_id":"<uuid>"}` à `/functions/v1/ingest_pubmed`, qui accepte désormais le paramètre body optionnel pour traiter une source unique par invocation. Body legacy `{}` conservé (rétrocompat, plus utilisé par les crons). Convention `body := %L::jsonb` héritée de 20260423. Source IDs récupérés via MCP Supabase à la date de la migration ; toute nouvelle source pubmed nécessitera un cron supplémentaire (ticket dédié). |
| 2026-05-02 | `20260502_sprint1_rbac_multitenant.sql` (+ rollback) | `claude/rbac-multi-tenant-setup-d0NC3` | Sprint 1 / Ticket 1 — fondations BDD multi-tenant + RBAC. 5 enums (`app_role`, `org_type`, `intra_role`, `org_plan`, `membership_status`) + 3 tables (`user_roles`, `organizations`, `organization_members`) + trigger `validate_intra_role_matches_org_type` (V1.2 : `assistante` valide dans les 3 types) + 4 helpers SQL `SECURITY DEFINER` (`has_role`, `is_super_admin`, `user_org`, `org_can_create_content`) avec `REVOKE FROM PUBLIC` + `GRANT TO authenticated, service_role` + 11 policies RLS (pas de DELETE sur `organization_members` → soft-delete via `status='revoked'`) + seed `super_admin` Dr Fantin (orgless en V1, décision Q2 matrice). Aligné sur `MATRICE_ROLES_DENTALLEARN_V1.md` V1.2 et `handoff_claude_code_sprint1_auth_rbac_v1_0.md`. |
