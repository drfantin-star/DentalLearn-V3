    ...
    <main className="mx-auto max-w-4xl p-6 text-white">
      <header className="mb-6 space-y-2">
        ...
        <h1 className="text-2xl font-bold">
          Démo <code className="rounded bg-white/10 px-2 py-0.5 text-base">{'<EnrichedAudioPlayer>'}</code>
        </h1>
      ...
      {!error && sequences.length === 0 && (
        <p className="rounded-lg border border-white/10 bg-white/5 p-4 text-sm text-white/70">
          Aucune séquence avec <code>timeline_url</code> défini en base. Lance
          d'abord le pipeline T2 ou enregistre une URL via{' '}
          <Link href="/admin/timelines" className="text-emerald-300 underline">
            /admin/timelines
          </Link>
          .
        </p>
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="space-y-1">
                  <p className="text-base font-semibold">{seq.title}</p>
      ...
      <footer className="mt-8 text-xs text-white/50">
        <p>
          ...
          <code>AudioPlayer.tsx</code>, ni à <code>SequencePlayer.tsx</code>.
          Voir <code>RAPPORT_T7_2_DEMO_ENRICHED_PLAYER.md</code>.
        </p>
      </footer>
    </main>
  )
