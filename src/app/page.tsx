...
const quizQuestions: QuizQuestion[] = [
  {
    ...
    isTrue: true,
    explanation: "Vrai. Depuis 2026, la fréquence passe à 3 fois par an pour les patients en ALD diabète.",
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center px-3 py-1.5 rounded-xl transition-all ${
                isActive ? 'bg-gradient-to-b from-ds-blue/10 to-ds-turquoise/10' : 'hover:bg-gray-50'
              }`}
            >
              <Icon 
                size={22} 
                className={isActive ? 'text-ds-blue' : 'text-gray-400'}
                strokeWidth={isActive ? 2.5 : 2}
              />
              <span className={`text-[10px] mt-1 font-medium ${isActive ? 'text-ds-blue' : 'text-gray-400'}`}>
                {tab.label}
              </span>
            </button>
          )
              ...
              <div className="flex-1 flex gap-1">
                {segments.map((seg) => (
                  <div 
                    ...
                    style={{ backgroundColor: seg < axis.progressFilled ? axis.color : '#F1F5F9' }}
                  />
                ))}
              </div>
            </div>
...
// Carte Entraînement
function TrainingCard({ axis, onStartQuiz }: { axis: AxisData; onStartQuiz: (axis: AxisData) => void }) {
  const Icon = axis.icon
  return (
    <button
      ...
      className={`w-full bg-white rounded-2xl p-4 shadow-sm border border-gray-100 text-left transition-all ${
        axis.dailyDone ? 'opacity-75' : 'hover:shadow-md hover:scale-[1.02] active:scale-[0.98]'
      }`}
    >
      ...
      <div className="mt-3 h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
        <div 
          ...
          style={{ width: axis.dailyDone ? '100%' : '0%', backgroundColor: axis.color }}
        />
      </div>
    </button>
  )
...
function NewsSection({ news }: { news: NewsItem[] }) {
  const getCategoryStyle = (category: NewsItem['category']) => {
    switch (category) {
      case 'reglementaire': return { icon: Scale, bg: 'bg-blue-50', text: 'text-blue-600', label: 'Réglementaire' }
      case 'scientifique': return { icon: FlaskConical, bg: 'bg-purple-50', text: 'text-purple-600', label: 'Scientifique' }
      case 'pratique': return { icon: Stethoscope, bg: 'bg-teal-50', text: 'text-teal-600', label: 'Pratique' }
      case 'humour': return { icon: PartyPopper, bg: 'bg-pink-50', text: 'text-pink-600', label: 'Humour' }
    }
  }
    ...
    <section>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
          <Newspaper size={20} className="text-ds-blue" />
          Veille métier
        </h2>
        <button className="text-xs font-bold text-ds-blue flex items-center gap-1 hover:underline">
            ...
            >
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h3 className="text-sm font-semibold text-gray-900 leading-snug line-clamp-2 mb-1">{item.title}</h3>
...
function CurrentFormationCard() {
  return (
    <div className="relative bg-gradient-to-br from-ds-blue to-ds-blue-dark rounded-2xl p-5 overflow-hidden">
      <div className="absolute -top-8 -right-8 w-32 h-32 bg-ds-turquoise opacity-20 rounded-full" />
      <div className="absolute -bottom-4 -left-4 w-20 h-20 bg-ds-turquoise opacity-10 rounded-full" />
      <div className="relative z-10">
        <span className="text-ds-turquoise text-xs font-bold uppercase tracking-wide">En cours</span>
        ...
        <div className="h-2 bg-white/20 rounded-full mb-4 overflow-hidden">
          <div className="h-full bg-ds-turquoise rounded-full w-[33%]" />
        </div>
        <button className="flex items-center gap-2 px-4 py-2.5 bg-ds-turquoise text-white rounded-xl text-sm font-bold hover:bg-ds-turquoise-dark transition-colors">
...
// Modal Quiz
function DailyQuizModal({ axis, onClose, onComplete }: { axis: AxisData; onClose: () => void; onComplete: () => void }) {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-gray-900/70 backdrop-blur-sm">
        <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl text-center">
          ...
          <div className="space-y-3">
            <button onClick={onComplete} className="w-full py-3.5 bg-gradient-to-r from-ds-blue to-ds-turquoise text-white rounded-xl font-bold flex items-center justify-center gap-2">
              Continuer <ArrowRight size={18} />
            </button>
            <button onClick={() => { onComplete(); onClose() }} className="w-full py-3.5 bg-gray-100 text-gray-600 rounded-xl font-bold">
              Retour à l'accueil
            </button>
          </div>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-gray-900/70 backdrop-blur-sm">
      <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden">
        <div className="h-1.5 bg-gray-100">
          <div className="h-full transition-all duration-500" style={{ width: `${((currentQuestion + 1) / quizQuestions.length) * 100}%`, backgroundColor: axis.color }} />
        </div>
        <div className="p-4 pb-0 flex justify-between items-start">
          <div>
            <div className="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg text-xs font-bold mb-2" style={{ backgroundColor: `${axis.color}15`, color: axis.color }}>
              <axis.icon size={14} /> {axis.shortName}
            </div>
            <div className="text-xs text-gray-400">Question {currentQuestion + 1}/{quizQuestions.length}</div>
          </div>
          <button onClick={onClose} className="p-2 text-gray-300 hover:text-gray-500 rounded-full">
            <X size={20} />
          </button>
        </div>
        <div className="p-6 pt-4">
          <h3 className="text-lg font-bold text-gray-900 leading-snug mb-6 min-h-[80px]">{currentQ.text}</h3>
          {!showFeedback ? (
            <div className="grid grid-cols-2 gap-4">
              <button onClick={() => handleAnswer(true)} className="h-28 rounded-2xl bg-emerald-50 border-2 border-emerald-100 flex flex-col items-center justify-center gap-2 hover:bg-emerald-100 transition-all active:scale-95">
              ...
              <button onClick={() => handleAnswer(false)} className="h-28 rounded-2xl bg-red-50 border-2 border-red-100 flex flex-col items-center justify-center gap-2 hover:bg-red-100 transition-all active:scale-95">
            ...
            <div className={`rounded-2xl p-5 ${isCorrect ? 'bg-emerald-50 border border-emerald-100' : 'bg-red-50 border border-red-100'}`}>
              ...
              {currentQ.source && <p className="text-[11px] text-gray-400 italic mb-4">{currentQ.source}</p>}
              <button onClick={nextQuestion} className="w-full py-3 bg-gray-900 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2">
                {currentQuestion < quizQuestions.length - 1 ? 'Suivant' : 'Voir mon score'} <ArrowRight size={16} />
              </button>
            </div>
          )}
        </div>
...
export default function HomePage() {
  ...
  const handleQuizComplete = () => {
    if (selectedAxis) {
      setAxes(prev => prev.map(a => a.id === selectedAxis.id ? { ...a, dailyDone: true } : a))
    }
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <div className="max-w-lg mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-full bg-gradient-to-br from-ds-turquoise to-ds-blue p-0.5">
      ...
      <main className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
              <Sparkles size={20} className="text-ds-turquoise" />
              Entraînement du jour
            </h2>
            <span className="text-xs font-bold text-gray-400">{dailyDoneCount}/4</span>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {axes.map((axis) => (
              <TrainingCard key={axis.id} axis={axis} onStartQuiz={handleStartQuiz} />
            ))}
          </div>
        </section>
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
              <GraduationCap size={20} className="text-ds-blue" />
              Ma formation
            </h2>
            <button className="text-xs font-bold text-ds-blue flex items-center gap-1 hover:underline">
      ...
      {showDailyQuiz && selectedAxis && (
        <DailyQuizModal axis={selectedAxis} onClose={() => { setShowDailyQuiz(false); setSelectedAxis(null) }} onComplete={handleQuizComplete} />
      )}
    </div>
  )
}
