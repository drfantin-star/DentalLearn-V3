...
export default function EppPage() {
  ...
  // ============================================
  // PDF — Plan d'actions T1 (logique partagée dans src/lib/epp)
  // ============================================

  const generatePlanActionsPDF = async () => {
    const t1Sess = sessions.find(s => s.tour === 1)
    await generatePlanActionsPDFUtil({
      audit: { title: audit!.title, slug: audit!.slug },
      t1Session: {
        completed_at: t1Sess?.completed_at ?? null,
        nb_dossiers: t1Sess?.nb_dossiers ?? null,
        score_global: t1Sess?.score_global ?? null,
      },
      criteria,
      responses,
      suggestions,
