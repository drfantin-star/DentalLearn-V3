...
/**
 * Éditeur frise chronologique (POC-T6.3) — naming `TimelineTemplateEditor`
 * pour éviter la collision avec le type racine `Timeline`.
 *
...
export function TimelineTemplateEditor({ template: rawTemplate, onChange }: Props) {
      ...
      {events.map((ev, idx) => (
        ...
        >
          ...
          <input
            type="text"
            value={ev.at_label}
            onChange={(e) => setEvent(idx, { ...ev, at_label: e.target.value })}
