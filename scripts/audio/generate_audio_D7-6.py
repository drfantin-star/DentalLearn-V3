# generate_audio.py — D7-6 (Phase 2B POC-T2 + injection header Xing)
...
#
# SORTIE :
#   - sequence_XX.mp3                                (toujours, header Xing injecté)
...
#
# D7-6 — INJECTION HEADER XING POST-CONCAT :
#   La concaténation byte-level de chunks MP3 ElevenLabs produit un fichier
#   sans header Xing/LAME global. Conséquences observées (cf. RECAP T7.0-T7.1) :
#     - mutagen retourne une durée fausse (98s sur 538s pour la pilote)
#     - seek HTML5 désynchronisé entre currentTime et sortie audio
#     - ffmpeg logue 5 erreurs "Header missing" aux jonctions Sophie↔Martin
#   Fix : après écriture de la concat brute, on re-mux via ffmpeg avec
#   `-c:a libmp3lame -b:a 128k -ar 44100 -ac 1 -write_xing 1` (flags pivots
#   identiques au fix manuel T7.1 validé empiriquement 6/6 GO smoke browser).
#   Dépendance externe : ffmpeg (vérifié au lancement, installation macOS via
#   `brew install ffmpeg`).
...
def check_ffmpeg_available():
    """Vérifie la disponibilité de ffmpeg au lancement (fail-fast)."""
    if shutil.which("ffmpeg") is None:
        raise RuntimeError(
            "ffmpeg est requis pour l'injection du header Xing post-concat (D7-6).\n"
            "Installation macOS : brew install ffmpeg"
        )


def inject_xing_header(mp3_path):
    """
    Re-mux le MP3 concaténé via ffmpeg pour injecter un header Xing valide.

    Reprend les flags validés en T7.1 (smoke browser preview Vercel 6/6 GO,
    cf. RECAP_SESSION_POC_AUDIO_T7_0_T7_1_09MAI2026.md §3) :
        -c:a libmp3lame -b:a 128k -ar 44100 -ac 1 -write_xing 1

    Le re-encode CBR 128k → CBR 128k assainit aussi les éventuels frames
    corrompus aux jonctions de chunks (les 5 "Header missing" documentés).

    Retourne {duration_seconds, bitrate, channels, xing_injected}.
    """
    from mutagen.mp3 import MP3

    mp3_path = Path(mp3_path)
    tmp_path = mp3_path.with_name(mp3_path.stem + ".tmp_preXing" + mp3_path.suffix)

    # Renommer la concat brute vers .tmp_preXing pour libérer le nom final
    ...
    try:
        ...
        if tmp_path.exists() and not mp3_path.exists():
            tmp_path.rename(mp3_path)
        stderr_tail = e.stderr.decode("utf-8", errors="ignore")[-500:]
        raise RuntimeError(f"ffmpeg a échoué lors de l'injection Xing :\n{stderr_tail}") from e
...
def print_post_xing_summary(mp3_path, xing_info):
    """
    Résumé console post-Xing + hint SQL backfill (sous-tâche TR-3, Option α).
    Le sequence_id est volontairement un placeholder texte : copy-paste vers
    Supabase SQL Editor après remplacement manuel par l'UUID approprié.
    """
    duration = xing_info["duration_seconds"]
    human = format_duration_human(duration)
    print()
    print(f"✅ MP3 généré : {mp3_path}")
    print(f"   Durée      : {human} ({duration:.2f}s)")
    print(f"   Xing       : injecté ✅")
    print()
    print(f"   → Backfill SQL suggéré (à exécuter manuellement si pertinent) :")
    print(f"   UPDATE sequences SET course_duration_seconds = {int(round(duration))} WHERE id = '<sequence_id>';")


# --- MAIN ---
if __name__ == "__main__":
    # D7-6 : fail-fast si ffmpeg absent (requis pour l'injection Xing finale)
    ...
    if not WITH_TIMESTAMPS:
        ...
        # D7-6 : injection du header Xing post-concat
        xing_info = inject_xing_header(output_file)

        print_post_xing_summary(output_file, xing_info)
    ...
    print(f"   → {len(segments)} segments speakers, {total_words} mots, alignment {duration_sec:.2f}s")
